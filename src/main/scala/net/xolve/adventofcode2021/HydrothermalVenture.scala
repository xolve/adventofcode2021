...
object HydrothermalVenture {
  ...
  def parseInputFile(file: String): Seq[Line] = {
    ...
    Using(Source.fromFile(file)) { f =>
      f.getLines().map {
        case s"$x0,$y0 -> $x1,$y1" => Line.create(Point(x0.toInt, y0.toInt), Point(x1.toInt, y1.toInt))
        case line => throw new IllegalArgumentException(s"$line is in invalid format.")
      }
      .toSeq
    }.get
  }
    ...
    lines
      .flatMap( line =>
        if line.p0.x == line.p1.x then
          ...
          throw Exception(s"Cannot figure out line $line")
      )
      .groupBy(p => p)
      .count(_._2.length > 1)
  }
